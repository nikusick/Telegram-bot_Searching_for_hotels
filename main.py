...
def take_number_of_posts(city):
    ...
    bot.register_next_step_handler(number, take_destination_id, city.text)


def take_destination_id(number_of_posts, city):
    ...
    for el in response.json()['suggestions'][0]['entities']:
        if el['type'] == 'CITY' and city.lower() == el['name'].lower():
            ...
            photos = bot.send_message(number_of_posts.chat.id, 'Do you wanna photos? ')
            bot.register_next_step_handler(photos, add_photos, destination_id, number_of_posts.text)
            break
    else:
        bot.send_message(city.chat.id, 'I cant find this city!')


def add_photos(photos, destination_id, number_of_posts):
    if photos.text.lower() == 'yes':
        num = bot.send_message(photos.chat.id, 'How many? ')
        bot.register_next_step_handler(num, result_w_photos, destination_id, number_of_posts)

    else:
        result(photos, destination_id, number_of_posts)


def result_w_photos(num, destination_id, number_of_posts):
    url = "https://hotels4.p.rapidapi.com/properties/list"

    querystring = {"destinationId": destination_id, "pageNumber": "1", "pageSize": number_of_posts,
                   "checkIn": "2020-01-08",
    ...
    for el in response.json()['data']['body']['searchResults']['results']:
        ...
        photo_response = requests.request("GET", url, headers=headers, params=querystring)
        photos = str()
        for index in range(int(num.text)):
            photos += str(photo_response.json()['hotelImages'][index]['baseUrl'][:-11]) + '.jpg' + ' '

        bot.send_message(num.chat.id,
            f'Name: {el["name"]} '
            f'\nStreet address: {el["address"]["streetAddress"]} '
            f'\nDestination to centre: {el["landmarks"][0]["distance"]} '
            f'\nPrice: {el["ratePlan"]["price"]["current"]}'
            f'\nPhotos: {photos}')


def result(photos, destination_id, number_of_posts):
    url = "https://hotels4.p.rapidapi.com/properties/list"
