...
def get_day_in_ask_day_out(c):
    ...
    if not result and key:
        ...
        bot.send_message(c.message.chat.id,
                         'Дата выезда', reply_markup=calendar)
    ...
    if not result and key:
        ...
        with bot.retrieve_data(c.from_user.id, c.message.chat.id) as data:
            ...
            if data['command'] == '/custom':
                bot.set_state(c.from_user.id,
                              CustomStates.min_price, c.message.chat.id)
...
@bot.message_handler(state=[CustomStates.count_of_notes,
